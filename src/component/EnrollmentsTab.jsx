...
import { getStatusColor, getStatusLabel, exportEnrollmentsToExcel } from './adminHelpers';

const INITIAL_FILTERS = { search: '', status: '', formation: '', dateFrom: '', dateTo: '' };

export default function EnrollmentsTab({ enrollments, onSearch, onStatusChange }) {
    ...
    const formations = [...new Set(
        enrollments
            .map((e) => e.diploma?.name || e.diploma_name)
            .filter(Boolean)
    )].sort();
                ...
                <form onSubmit={handleSubmit} className="mb-6 space-y-4">
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4">
                        ...
                        <div className="xl:col-span-2">
                            <label className="block text-xs font-medium text-gray-500 mb-1">Nom / Email</label>
                            <input
                                type="text"
                        ...
                        {/* Formation */}
                        <div>
                            <label className="block text-xs font-medium text-gray-500 mb-1">Formation (diplôme)</label>
                            <select
                        ...
                        <div className="flex flex-col gap-1">
                            <label className="block text-xs font-medium text-gray-500">Période</label>
                ...
                <div className="overflow-x-auto">
                    <table className="min-w-full divide-y divide-gray-200">
                        <thead className="bg-gray-50">
                            <tr>
                                {['Étudiant', 'Diplôme', 'Filière', 'Statut', 'Date', 'Actions'].map((h) => (
                                ...
                                enrollments.map((enrollment) => (
                                    <tr key={enrollment.id} className="hover:bg-gray-50">
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                            {enrollment.diploma?.name || enrollment.diploma_name || '-'}
                                        </td>
                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
